...
foreach ($slo in $SLOs)
{
    ...
    if ($sm19SLOs.displayname -contains  $slo.displayname)
    {
    ...
    # [threading.thread]::CurrentThread.CurrentCulture = 'en-US'
        ...
        foreach ($rel in $slorels)
        {
          if ($rel.IsDeleted -eq $false)
          {
            ...
            $targetclass = Get-SCSMClass -Instance $relobj -ComputerName $sm12server
            Write-Host $targetclass.DisplayName
 
 #   Let's try to use #FullName to identify objects
         
 #           if ($relclass.DisplayName -eq 'Metric')
            {
  #                $sm12target = Get-SCSMClassInstance -class $targetclass -ComputerName $sm12server `
  #                          | ? '#FullName' -eq $($relobj.FullName)
            }
 <#
           else
...
# Need to get the real class since the previous one from relationship is abstract.                 
 #           $targetclass = Get-SCSMClass -Instance $sm12target -ComputerName $sm12server
